...
Для тех, кто не слышал про [d3.js][23], напомню, что это библиотека для визуализации данных,  обладающая большой гибкостью и имеющая широкий
спектр применения.

В этой статье я хочу пошагово рассказать как сделать простую визуализацию
на географической карте при помощи связки d3.js и [TopoJSON][24]. Будут затронуты
...
будем обращаться к shapefile — популярному формату геоданных. Shapefile позволяет хранить точки, линии, полигоны и другие объекты.
...
Если вы выбрали бинарный пакет nodejs, при установке возможна такая проблема:
...
Сначала нам нужно получить из shapefiles конечный
...
годы. Нужную информацию в приемлемом формате можно найти на [Freedom House][9]. 
...
Формализация данных, пожалуй, самое долгое и неинтересное занятие, но этот этап нельзя пропустить.
...
предварительно объявив в `window.onload` в анонимной функции несколько переменных
...
Добавим данные в функцию `processData` перед вызовом `drawMap(world)`:
...
годам.
Слайдер состоит из нескольких независимых компонентов:
...
визуализаций на картах в d3.js, была достигнута. В комментариях можно задать дополнительные вопросы, оставить предложения и критику, буду рад!
...
[23]: https://d3js.org/
[24]: https://github.com/mbostock/topojson
