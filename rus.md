...
Для тех, кто не слышал про d3.js, напомню, что это библиотека для визуализации данных,  обладающая большой гибкостью и имеющая широкий
спектр применения.
...
вопросы конвертации данных и их последующая загрузка,  также рассмотрим стилизацию карты и создание интерактивных элементов.
...
будем обращаться к shapefile —  популярному формату географических
данных. Shapefile позволяет хранить точки, линии, полигоны и другие объекты.
...
файлов: *.shp, *.shx и *.dbf. Важно, чтобы все три файла были расположены в одной директории.
...
Для topojson нам понадобится nodejs, его можно найти  в виде пакета в
репозитории.
...
Если вы выбрали бинарный пакет nodejs, то при установке возможна такая проблема:
...
TopoJSON-файл. Для этого нам понадобится сгенерировать промежуточный файл GeoJSON. На этапе генерации GeoJSON-файла мы получаем возможность отфильтровать
из shapefiles данные, которые нам не нужны, и уменьшить в значениях
число знаков после запятой, что важно для увеличения
скорости рендеринга, да и просто сокращает размер файла примерно на 85%.
...
Также нам понадобятся данные о свободе прессы (Freedom of the Press) за последние
годы. Хорошие данные в приемлемом формате можно найти на [Freedom House][9]. 
Данные представляют собой условный индекс свободы прессы
от 0 до 100, где 0 — наиболее свободные страны; 100 — наименее свободные.
...
Формализация данных, пожалуй, самое неинтересное и длительное по времени занятие, но этот этап нельзя пропустить.
...
так же добавим в `<body>` элемент `<div id="map"></div>`, в него и будет
помещён `svg`-элемент. В `<head>` надо добавить ещё несколько скриптов:
...
Добавим данные в код в функцию `processData` перед вызовом `drawMap(world)`:
...
данных за текущий год или метрика вовсе не производилась или там совсем нет прессы :)
...
каждой страны. Если данные не найдены, то страна будет окрашена в белый
цвет.
...
Функция `addSlider` состоит из 3-х частей, как описано выше. Её вызов
...
адаптирован под наш проект. Также нужно не
забыть подключить соответствующие стили и скрипты.
...
Напоследок хотел бы добавить всплывающие подсказки для каждой страны. Подсказка
...
визуализаций на картах в d3.js, была достигнута. В комментариях можно задать дополнительные вопросы, оставить предложения и критику (конструктивную), буду рад!
