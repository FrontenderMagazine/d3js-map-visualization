window.onload = function () {
  ...
  function loadData() {
    queue()   // queue function loads all external data files asynchronously 
      .defer(d3.json, "data/countries.json")  // our geometries
      .defer(d3.csv, "data/freedom.csv")  // and associated data in csv file
  ...
  function processData(error, world, countryData) {
    ...
    for (var i in countries) {    // for each geometry object
      for (var j in countryData) {  // for each row in the CSV
        ...
        if(countries[i].properties.iso_a3 == countryData[j].ISO3166) {   // if they match
          for(var k in countryData[i]) {   // for each column in the a row within the CSV
            if(k != 'Country' && k != 'ISO3166') {  // let's not add the name or id as props since we already have them
